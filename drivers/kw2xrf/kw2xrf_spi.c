...
void kw2xrf_write_dreg(kw2xrf_t *dev, uint8_t addr, uint8_t value)
{
    ...
    spi_transfer_reg(dev->params.spi, addr, value, NULL);
...
uint8_t kw2xrf_read_dreg(kw2xrf_t *dev, uint8_t addr)
{
    uint8_t value;
    kw2xrf_spi_transfer_head(dev);
    spi_transfer_reg(dev->params.spi, (addr | MKW2XDRF_REG_READ),
                     0x0, (char *)&value);
    kw2xrf_spi_transfer_tail(dev);
    return value;
}
...
void kw2xrf_write_iregs(kw2xrf_t *dev, uint8_t addr, uint8_t *buf, uint8_t length)
{
    ...
    kw2xrf_spi_transfer_head(dev);
    spi_transfer_regs(dev->params.spi, MKW2XDM_IAR_INDEX,
...
void kw2xrf_read_iregs(kw2xrf_t *dev, uint8_t addr, uint8_t *buf, uint8_t length)
{
    ...
    kw2xrf_spi_transfer_head(dev);
    spi_transfer_regs(dev->params.spi, MKW2XDM_IAR_INDEX | MKW2XDRF_REG_READ,
...
{
    kw2xrf_spi_transfer_head(dev);
    spi_transfer_regs(dev->params.spi, MKW2XDRF_BUF_WRITE,
...
{
    kw2xrf_spi_transfer_head(dev);
    spi_transfer_regs(dev->params.spi, MKW2XDRF_BUF_READ, NULL,
                      (char *)data, length);
    kw2xrf_spi_transfer_tail(dev);
}
/** @} */
