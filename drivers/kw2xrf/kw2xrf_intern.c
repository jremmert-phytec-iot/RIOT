...
{
    kw2xrf_write_dreg(MKW2XDM_OVERWRITE_VER, overwrites_direct[0].data);
    for (uint8_t i = 0; i < sizeof(overwrites_indirect)/sizeof(overwrites_t); i++) {
        kw2xrf_write_iregs(overwrites_indirect[i].address,
...
void kw2xrf_set_out_clk(kw2xrf_t *dev)
{
    ...
    uint8_t tmp = kw2xrf_read_dreg(MKW2XDM_CLK_OUT_CTRL);
    if (tmp != 0x8Bu) {
...
void kw2xrf_set_power_mode(kw2xrf_t *dev, kw2xrf_powermode_t pm)
{
    ...
    kw2xrf_write_dreg(MKW2XDM_PWR_MODES, reg);
}

int kw2xrf_can_switch_to_idle(kw2xrf_t *dev)
{
    uint8_t state = kw2xrf_read_dreg(MKW2XDM_SEQ_STATE);
    uint8_t seq = kw2xrf_read_dreg(MKW2XDM_PHY_CTRL1) & MKW2XDM_PHY_CTRL1_XCVSEQ_MASK;
...
{
    kw2xrf_write_dregs(MKW2XDM_T1CMP_LSB, (uint8_t *)&value, sizeof(value));
...
static uint32_t kw2xrf_timer_get(kw2xrf_t *dev)
{
    (void)dev;
    uint32_t tmp;
    kw2xrf_read_dregs(MKW2XDM_EVENT_TIMER_LSB, (uint8_t*)&tmp, sizeof(tmp));
    return tmp;
}
...
{
    (void)dev;
    kw2xrf_write_dregs(cmp_reg, (uint8_t *)&value, 3);
}
...
void kw2xrf_timer_init(kw2xrf_t *dev, kw2xrf_timer_timebase_t tb)
{
    ...
    kw2xrf_write_iregs(MKW2XDMI_TMR_PRESCALE, &tmp, 1);
...
void kw2xrf_trigger_tx_ops_disable(kw2xrf_t *dev)
{
    ...
    kw2xrf_write_dreg(MKW2XDM_IRQSTS3, MKW2XDM_IRQSTS3_TMR2IRQ);
    uint32_t now = kw2xrf_timer_get(dev);
...
void kw2xrf_abort_rx_ops_disable(kw2xrf_t *dev)
{
    ...
    kw2xrf_write_dreg(MKW2XDM_IRQSTS3, MKW2XDM_IRQSTS3_TMR3IRQ);
    uint32_t now = kw2xrf_timer_get(dev);
    DEBUG("[kw2xrf] now: %" PRIx32 "\n", now);
}
...
void kw2xrf_seq_timeout_on(kw2xrf_t *dev, uint32_t timeout)
{
    ...
    uint8_t irqsts3 = kw2xrf_read_dreg(MKW2XDM_IRQSTS3) & 0xf0;
    irqsts3 &= ~MKW2XDM_IRQSTS3_TMR4MSK;
    irqsts3 |= MKW2XDM_IRQSTS3_TMR4IRQ;
    kw2xrf_write_dreg(MKW2XDM_IRQSTS3, irqsts3);
...
void kw2xrf_seq_timeout_off(kw2xrf_t *dev)
{
    ...
    kw2xrf_write_dreg(MKW2XDM_IRQSTS3, MKW2XDM_IRQSTS3_TMR4IRQ);
    uint32_t now = kw2xrf_timer_get(dev);
    DEBUG("[kw2xrf] now: %" PRIx32 "\n", now);
}
...
{
    uint32_t tmp;
    kw2xrf_read_dregs(MKW2XDM_TIMESTAMP_LSB, (uint8_t*)&tmp, sizeof(tmp));
    return tmp;
}
